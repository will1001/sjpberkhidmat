...
const DetailKelurahan = () => {
  ...
  useEffect(() => {
    router.query.display === "mobile" &&
      axiosFetch("get", `user/dashboard/statistik/kelurahan/${filterKecamatan}?page=1&limit=10&sort=${filterUrutan}${keyword !== undefined && keyword.length >= 3 ? `&keyword=${keyword}` : ""}`)
        .then((res) => setData(res.data))
        .catch((err) => console.log(err));
  }, [filterKecamatan, keyword, filterUrutan, currentPage]);
  console.log(data);
  return (
    <>
      {router.query.display === "mobile" ? (
        <div className="px-[16px] pt-[24px] pb-[50px]">
          <div className="flex">
          ...
          <p className="text-[32px] text-[#374151] font-bold mt-[21px]">Peta Kekuatan Per Desa / Kelurahan</p>
          <div className="mt-4">
            <p className="text-[#9CA3AF] font-medium">DAFTAR KECAMATAN</p>
            <select value={filterKecamatan} onChange={(e) => setFilterKecamatan(e.target.value)} className="border border-[#9CA3AF] text-[18px] font-semibold mt-2 outline-0 cursor-pointer w-full px-2 py-2 rounded-md">
          ...
          <div className="stroke-[#374151] w-full mt-3 px-2 flex items-center border rounded-sm h-[43px] justify-between">
            <input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="Cari Desa / Kelurahan" className="border-none outline-0 w-full" type={"text"} />
            <SearchIcon />
          </div>
          <div className="flex justify-between items-center">
            <p className="text-[18px]">Urutkan</p>
            <div>
              <div onClick={() => setPopupFilter(true)} className="flex mt-3 gap-3 rounded-md cursor-pointer stroke-white bg-[#FF5001] py-2 px-4 text-white">
                <FilterIcon />
                <p className="font-medium">Filter Data</p>
              </div>
              {popupFilter === true && (
                <div className="absolute bg-white w-[200px] pt-3">
                  <p
                    onClick={() => {
                      setPopupFilter(false);
                      setFilterUrutan("terkecil");
                    }}
                    className={`text-center w-[140px] justify-center font-medium ${filterUrutan === "terkecil" && "text-[#E44700]"}`}
                  >
                    terkecil
                  </p>
                  <p
                    onClick={() => {
                      setPopupFilter(false);
                      setFilterUrutan("terbanyak");
                    }}
                    className={`text-center w-[140px] justify-center font-medium ${filterUrutan === "terbanyak" && "text-[#E44700]"}`}
                  >
                    terbanyak
                  </p>
                </div>
          ...
          {filterKecamatan === undefined ? (
            <div className="text-[24px] text-[#374151] text-center font-semibold mt-[24px]">Silahkan Pilih Kecamatan</div>
          ) : (
              ...
              <table className="min-w-full overflow-auto">
                <thead className="bg-[#374151]  ">
                  <tr className="h-[51px] text-white">
                    <th scope="col" className="px-6  py-3 whitespace-nowrap text-left text-xs font-medium ">
                      No
                    </th>
                    <th scope="col" className="px-6  py-3 whitespace-nowrap text-left text-xs font-medium">
                      Desa / Kelurahan
          ...
          <Pagination mobile={true} current_page={currentPage} setCurrentPage={setCurrentPage} total={data?.metadata?.total} total_page={data?.metadata?.totalPage} />
        </div>
      ) : (
        <>
          {" "}
          <div style={popupFilter === true ? { visibility: "visible" } : { visibility: "hidden" }} className="fixed top-0 left-0 w-screen h-screen bg-[#37415152] z-50">
            <div className="absolute bg-white left-[450px] top-[100px] text-[#374151] p-[60px]">
              ...
              Batal
                ...
                <div onClick={() => setPopupFilter(false)} className="w-full text-white font-semibold rounded-md bg-[#E44700] text-[18px] py-2 px-6 flex justify-center cursor-pointer">
                  Terapkan Filter
                </div>
              </div>
            </div>
