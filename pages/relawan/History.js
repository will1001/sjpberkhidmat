...
import { ImportGambarIcon, KameraIcon, SearchIcon } from "../../src/utility/icon/icon";
...
const History = () => {
  ...
  useEffect(() => {
    if (keyword !== "" && keyword.length >= 3) {
      axiosFetch("get", `user/real_count/plano?page=1&limit=10&keyword=${keyword}`, {}, token)
      ...
      {screenSize.width >= 350 && screenSize.width <= 950 ? (
        <div>
              ...
              <div className="bg-white w-full px-[16px] pb-[50px] mt-[80px]">
                ...
                {openCamera === true && (
                  <div className="absolute top-0 left-0 h-screen w-screen bg-[#374151] z-50">
                    <Camera setUploadPlano={setUploadPlano} screenSize={screenSize} setOpenCamera={setOpenCamera} />
                  </div>
                )}
              </div>
          ...
          <div className="px-[16px]">
            <div onClick={() => router.back()} className="p-2 flex items-center w-[149px] justify-center gap-2 rounded-sm mt-[12px] cursor-pointer bg-[#374151]">
            ...
            <div className="flex items-center w-full border rounded-sm mt-3 py-2 px-2 stroke-black">
              <input onChange={(e) => setKeyword(e.target.value)} className="outline-0 w-full" type={"text"} id={"search"} placeholder="Cari Data" />
              <div className="h-[15px] w-[15px] flex items-center">
                <SearchIcon />
              </div>
            </div>
            <div onClick={() => setPopup(true)} className="flex mt-3 bg-[#E44700] items-center  justify-center gap-2 py-2 px-3 rounded-sm cursor-pointer">
            ...
            <div className="overflow-x-scroll scrollbar-thin scrollbar-thumb-[#374151]">
              ...
              <div className="">
                {planao?.data?.data?.map((res, i) => (
                  <div style={(i + 1) % 2 !== 0 ? { background: "#F9FAFB" } : { background: "white" }} className="flex py-[20px] items-center gap-3 p-2 mr-[50px] w-[900px]" key={res._id}>
                    <div className="w-[150px]">{res.image !== undefined && <img className="w-[80px] h-[80px]" src={process.env.NEXT_PUBLIC_BASE_URL_IMAGE + res.image} />}</div>
                    ...
                    <div className="w-[250px] flex justify-center gap-2 items-center">
                      ...
                      {alert === false ? (
                        ...
                        <div className="flex gap-3 font-medium">
                          ...
                          <div onClick={() => hapusPlano(res._id)} className="text-red-700 cursor-pointer">
                            Hapus
                          </div>
                        </div>
                      )}
        ...
        <>
          ...
          <div className="text-[#374151] p-6">
            <div className="flex gap-12 border-b-2">
              <Logo />
              <p className="text-[26px] font-semibold"> {roles === "relawan" ? "History Upload C1 Plano" : "Foto C Plano (Diupload Relawan)"}</p>
            </div>
            ...
            <div className="overflow-auto">
              ...
              <div className="w-screen">
                {planao?.data?.data?.map((res, i) => (
                  <div style={(i + 1) % 2 !== 0 ? { background: "#F9FAFB" } : { background: "white" }} className="flex py-[20px] items-center gap-3 p-2 mr-[50px] w-screen" key={res._id}>
