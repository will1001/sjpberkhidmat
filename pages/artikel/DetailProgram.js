    ...
    <>
      {screenSize.width >= 350 && screenSize.width <= 450 ? (
        <div>
          ...
          <div className="text-center text-[18px] text-white py-3 font-bold bg-[#FF5001]">Program</div>
          ...
          <div className="bg-[#4B5563] px-[16px] pb-[53px]">
            <p className="text-[21px] text-center text-white font-bold pt-[36px] ">Program SJP Berkhidmat</p>
            <div className="mt-6">
              {getArtikel?.data?.map((res) => {
                if (res.publication === true) {
                  return (
                    <div key={res._id} className="flex border h-[111px] rounded-sm mt-2">
                      <div className="text-white text-[10px] basis-2/3 py-[5px] px-[8px]">
                        <p className="text-[#FF5001] font-semibold">{res?.category}</p>
                        <p className="text-[12px] h-[32px] overflow-clip font-semibold">{res?.title}</p>
                        <p className="text-white text-[16px] h-[50px] w-[213px] overflow-clip break-words">
                          <span className="" dangerouslySetInnerHTML={{ __html: res.description }} />
                        </p>
                      </div>
                      <div
                        onClick={() => {
                          router.push({
                            pathname: "./DetailProgram",
                            query: {
                              title: res.title,
                        ...
                        className="basis-1/3 border-l flex items-center"
                      >
                        <img src={detailProgramImg.src} />
                      </div>
                    </div>
                  );
                }
        ...
        <div>
          <div className="fixed top-0 w-screen border-b-2 z-50 shadow-md">
            ...
            <div className="flex bg-white w-full h-[72px] px-20 justify-between z-20">
              ...
              <div className="flex items-center gap-4 text-[16px] font-medium text-[#374151]">
                ...
                <p className="cursor-pointer text-[#FF5001]">Program</p>
                <p
                  onClick={() =>
                    router.push({
                      pathname: "/",
          ...
          <div className="flex justify-center items-center bg-orange-500 mt-[75px] h-[100px]">
            <h1 className="font-bold text-2xl text-white">Program</h1>
          </div>
          <div className="px-[246px] py-[88px]  ">
            <div className="text-[#374151] text-[18px] font-medium flex gap-4">
              Daerah Tujuan Program <span className="text-[#FF5001] font-bold">{`${router.query.kabupaten}`}</span>
            </div>
            <br />
            <div className="text-[#374151] text-[52px] font-bold">{router.query.title}</div>

            {router.query.image !== null && <img className="py-3" src={process.env.NEXT_PUBLIC_BASE_URL_IMAGE + router.query.image} alt="" />}
            {router.query.video !== "" && (
              <div>
              ...
              <source
                src={
                  ...
                  router.query.video?.split("=").pop()
                }
                type="video/mp4"
              />
              Your browser does not support the video tag.
            </video> */}
                <iframe className="my-[20px]" width="800" height="500" src={"https://www.youtube.com/embed/" + router.query.video?.split("=").pop()}></iframe>
                <br />
              </div>
            )}
            <div className="text-[#374151]" dangerouslySetInnerHTML={{ __html: router.query.description }} />
          </div>
...
export default withRouter(DetailProgram);
