...
const InputPartai = ({ getPartai }) => {
  ...
  const postPartai = async () => {
    ...
    {
      await axiosFetch("post", `user/real_count/partai`, a, token)
        .then((res) => {
          console.log(res);
          console.log(a);
    ...
    <>
      <div className="flex pl-[41px] h-[72px] bg-white border border-[#D1D5DB] rounded-t-md">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[21px] text-[#374151] font-semibold">{getPartai?.data?.length !== 0 && (getPartai?.data?.length + 1).toString()}</p>
        </div>
