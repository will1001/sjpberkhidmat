    ...
    <div className="font-bold text-[32px] text-slate-700">
      ...
      <div className="flex mt-8">
        {data.length !== 0 && (
          ...
          //   width="420"
          //   height="315"
          //   src={"https://www.youtube.com/embed/" + data.video.split("=").pop()}
          // ></iframe>
        )}
      </div>
      <div className="flex items-center mt-3 gap-6">
        <p className="text-[#FF5001] text-[21px] font-medium">
          {data?.category}
        </p>
        <p className="text-[21px] font-medium">
          {data?.createdAt?.split("T").shift().split("-")[2]}{" "}
          {
            monthNames[
              new Date(
                data?.createdAt?.split("T").shift().split("-")[1]
              ).getMonth()
            ]
          }{" "}
          {data?.createdAt?.split("T").shift().split("-")[0]}
        </p>
      </div>
