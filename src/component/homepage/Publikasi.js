...
const Publikasi = ({ data, mobile }) => {
  ...
  return (
    <>
      {mobile !== undefined ? (
        <div className="font-bold px-[16px] text-[10px] text-slate-700">
          {data?.image && <img className="w-[328px] h-[164px] rounded-xl object-cover" src={process.env.NEXT_PUBLIC_BASE_URL_IMAGE + data?.image} alt="Frame-2519" border="0"></img>}
          <div className="flex justify-center mt-2">{data?.length !== 0 && <iframe className="rounded-md" width="328" height="164" src={"https://www.youtube.com/embed/" + data?.video?.split("=").pop()}></iframe>}</div>
          <div className="flex items-center mt-3 gap-6">
            <p className="text-[#FF5001] text-[10px] font-medium">{data?.category}</p>
